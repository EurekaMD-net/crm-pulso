...
const { isGoogleEnabled, getGmailClient, getCalendarClient } = await import('../src/google-auth.js');
...
describe('getGmailClient', () => {
  ...
  it('throws when env not set', () => {
    delete process.env.GOOGLE_SERVICE_ACCOUNT_KEY;
    expect(() => getGmailClient('user@example.com')).toThrow('GOOGLE_SERVICE_ACCOUNT_KEY not set');
  });
...
describe('getCalendarClient', () => {
  ...
  it('throws when env not set', () => {
    delete process.env.GOOGLE_SERVICE_ACCOUNT_KEY;
    expect(() => getCalendarClient('user@example.com')).toThrow('GOOGLE_SERVICE_ACCOUNT_KEY not set');
  });
