...
function setupDb() {
  ...
  // Seed personas
  testDb.prepare(`INSERT INTO persona (id, nombre, rol, whatsapp_group_folder, activo)
    VALUES ('ae-001', 'Carlos Lopez', 'ae', 'carlos-ae', 1)`).run();
  testDb.prepare(`INSERT INTO persona (id, nombre, rol, whatsapp_group_folder, activo)
    VALUES ('mgr-001', 'Ana Garcia', 'gerente', 'ana-mgr', 1)`).run();
  testDb.prepare(`INSERT INTO persona (id, nombre, rol, reporta_a, whatsapp_group_folder, activo)
    VALUES ('ae-002', 'Maria Perez', 'ae', 'mgr-001', 'maria-ae', 1)`).run();
  testDb.prepare(`INSERT INTO persona (id, nombre, rol, whatsapp_group_folder, activo)
    VALUES ('dir-001', 'Roberto Diaz', 'director', 'roberto-dir', 1)`).run();
  testDb.prepare(`INSERT INTO persona (id, nombre, rol, whatsapp_group_folder, activo)
    VALUES ('vp-001', 'Elena Ruiz', 'vp', 'elena-vp', 1)`).run();
}
...
describe('role-based tool filtering', () => {
  ...
  it('gerente gets 20 tools', () => {
    const tools = getToolsForRole('gerente');
    expect(tools.length).toBe(20);
  });

  it('director gets 19 tools', () => {
...
describe('system prompt construction', () => {
  ...
  it('combines global + group CLAUDE.md', () => {
    ...
    if (fs.existsSync(globalFile)) parts.push(fs.readFileSync(globalFile, 'utf-8'));
    const groupFile = path.join(groupPath, 'CLAUDE.md');
    if (fs.existsSync(groupFile)) parts.push(fs.readFileSync(groupFile, 'utf-8'));
    parts.push('\n## Tu Identidad\nNombre: Carlos Lopez\nRol: ae\nGrupo: carlos-ae');
...
describe('session management', () => {
  ...
  it('save and load session round-trip', () => {
    ...
    const loaded = JSON.parse(fs.readFileSync(filePath, 'utf-8')) as ChatMessage[];
...
describe('output marker format', () => {
  ...
  it('output is valid JSON between markers', () => {
    const output = { status: 'success', result: 'Hola', newSessionId: 'test-123' };
...
describe('session ID generation', () => {
  it('generates unique session IDs', () => {
    const generateSessionId = () => `crm-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
