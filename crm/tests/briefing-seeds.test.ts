...
vi.mock("cron-parser", () => ({
  CronExpressionParser: {
    parse: () => ({
      next: () => ({
        toISOString: () => new Date(Date.now() + 86400000).toISOString(),
      }),
    }),
  },
}));
...
const { seedBriefings, BRIEFING_SEEDS } =
  await import("../src/briefing-seeds.js");
...
function setupDb() {
  ...
  testDb
    .prepare(
      `INSERT INTO persona (id, nombre, rol, reporta_a, whatsapp_group_folder, activo) VALUES ('ger1', 'Miguel', 'gerente', null, 'ger-miguel', 1)`,
    )
    .run();
  testDb
    .prepare(
      `INSERT INTO persona (id, nombre, rol, reporta_a, whatsapp_group_folder, activo) VALUES ('ae1', 'Maria', 'ae', 'ger1', 'ae-maria', 1)`,
    )
    .run();
  testDb
    .prepare(
      `INSERT INTO persona (id, nombre, rol, reporta_a, whatsapp_group_folder, activo) VALUES ('ae2', 'Carlos', 'ae', 'ger1', 'ae-carlos', 1)`,
    )
    .run();
  // Inactive AE
  testDb
    .prepare(
      `INSERT INTO persona (id, nombre, rol, reporta_a, whatsapp_group_folder, activo) VALUES ('ae3', 'Pedro', 'ae', 'ger1', 'ae-pedro', 0)`,
    )
    .run();
  // AE without group folder
  testDb
    .prepare(
      `INSERT INTO persona (id, nombre, rol, reporta_a, whatsapp_group_folder, activo) VALUES ('ae4', 'Luis', 'ae', 'ger1', null, 1)`,
    )
    .run();
  // Director
  testDb
    .prepare(
      `INSERT INTO persona (id, nombre, rol, reporta_a, whatsapp_group_folder, activo) VALUES ('dir1', 'Ana', 'director', null, 'dir-ana', 1)`,
    )
    .run();
  // VP
  testDb
    .prepare(
      `INSERT INTO persona (id, nombre, rol, reporta_a, whatsapp_group_folder, activo) VALUES ('vp1', 'Roberto', 'vp', null, 'vp-roberto', 1)`,
    )
    .run();
  ...
  testDb
    .prepare(
      `INSERT INTO registered_groups (jid, name, folder, trigger_pattern, added_at) VALUES ('jid-ae1', 'AE Maria', 'ae-maria', '@bot', '2026-01-01')`,
    )
    .run();
  testDb
    .prepare(
      `INSERT INTO registered_groups (jid, name, folder, trigger_pattern, added_at) VALUES ('jid-ae2', 'AE Carlos', 'ae-carlos', '@bot', '2026-01-01')`,
    )
    .run();
  testDb
    .prepare(
      `INSERT INTO registered_groups (jid, name, folder, trigger_pattern, added_at) VALUES ('jid-ger1', 'Gerente Miguel', 'ger-miguel', '@bot', '2026-01-01')`,
    )
    .run();
  testDb
    .prepare(
      `INSERT INTO registered_groups (jid, name, folder, trigger_pattern, added_at) VALUES ('jid-dir1', 'Director Ana', 'dir-ana', '@bot', '2026-01-01')`,
    )
    .run();
  testDb
    .prepare(
      `INSERT INTO registered_groups (jid, name, folder, trigger_pattern, added_at) VALUES ('jid-vp1', 'VP Roberto', 'vp-roberto', '@bot', '2026-01-01')`,
    )
    .run();
}
...
describe("seedBriefings", () => {
  it("creates correct number of tasks per role", () => {
    ...
    const tasks = testDb
      ...
      .all() as any[];
    // AE: 3 seeds x 2 active AEs with groups = 6
  ...
  it("creates tasks with correct cron expressions", () => {
    ...
    expect(crons).toEqual(["0 16 * * 5", "10 9 * * 1-5", "30 18 * * 1-5"]);
  });

  it("skips inactive personas", () => {
    seedBriefings();

    const pedroTasks = testDb
      .prepare("SELECT * FROM scheduled_tasks WHERE group_folder = 'ae-pedro'")
      .all();
  ...
  it("skips personas whose group is not registered", () => {
    ...
    testDb
      .prepare("DELETE FROM registered_groups WHERE folder = 'ae-carlos'")
      .run();

    seedBriefings();

    const carlosTasks = testDb
      .prepare("SELECT * FROM scheduled_tasks WHERE group_folder = 'ae-carlos'")
      .all();
  ...
  it("all tasks have context_mode group", () => {
    ...
    const tasks = testDb
      .prepare("SELECT context_mode FROM scheduled_tasks")
      .all() as any[];
  ...
  it("all tasks have status active", () => {
    ...
    const tasks = testDb
      .prepare("SELECT status FROM scheduled_tasks")
      .all() as any[];
  ...
  it("all tasks have valid future next_run", () => {
    ...
    const tasks = testDb
      .prepare("SELECT next_run FROM scheduled_tasks")
      .all() as any[];
    const now = new Date();
    for (const t of tasks) {
      expect(t.next_run).toBeTruthy();
      expect(new Date(t.next_run as string).getTime()).toBeGreaterThan(
        now.getTime() - 60000,
      );
    }
  });
...
describe("briefing stagger order", () => {
  ...
  it("Gerente fires before AE morning", () => {
    const gerSeed = BRIEFING_SEEDS.find((s) => s.rol === "gerente")!;
    const aeMorningSeed = BRIEFING_SEEDS.find(
      (s) => s.rol === "ae" && s.cron.includes("1-5"),
    )!;
