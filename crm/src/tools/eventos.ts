...
import { getMxDateStr } from "./helpers.js";
...
export function consultar_eventos(
  ...
  const futureDate = getMxDateStr(
    new Date(Date.now() + diasAdelante * 86400000),
  );
  ...
  if (rows.length === 0) {
    return JSON.stringify({
      mensaje: "No hay eventos en el periodo indicado.",
    });
  }
  ...
  return JSON.stringify({
    ...
    eventos: rows.map((r) => {
      const diasPara = Math.ceil(
        (new Date(r.fecha_inicio).getTime() - Date.now()) / 86400000,
      );
      ...
      if (r.inventario_total && r.inventario_vendido) {
        try {
          ...
          disponibilidad =
            totalUnits > 0
              ? `${Math.round((1 - soldUnits / totalUnits) * 100)}% disponible`
              : undefined;
        } catch {
          /* ignore parse errors */
        }
...
export function consultar_inventario_evento(
  args: Record<string, unknown>,
  _ctx: ToolContext,
): string {
  const db = getDatabase();
  ...
  try {
    ...
    return JSON.stringify({
      error: "Error al procesar inventario del evento.",
    });
  }
