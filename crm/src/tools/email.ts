...
export function enviar_email_seguimiento(
  ...
  if (!contacto) {
    return JSON.stringify({
      error: `No encontré el contacto con ID "${contactoId}".`,
    });
  }
  if (!contacto.email) {
    return JSON.stringify({
      error: `El contacto "${contacto.nombre}" no tiene email registrado.`,
    });
  }
...
export async function confirmar_envio_email(
  args: Record<string, unknown>,
  ctx: ToolContext,
): Promise<string> {
  const db = getDatabase();
  ...
  const email = db
    .prepare("SELECT * FROM email_log WHERE id = ? AND persona_id = ?")
    .get(emailId, ctx.persona_id) as any;
  if (!email) {
    return JSON.stringify({
      error: `No encontré el email "${emailId}" o no tienes acceso.`,
    });
  }
  if (email.enviado === 1) {
  ...
  db.prepare("UPDATE email_log SET error = ? WHERE id = ?").run(
    "SMTP no configurado — guardado como borrador",
    emailId,
  );
  return JSON.stringify({
    ok: true,
...
export async function enviar_email_briefing(
  args: Record<string, unknown>,
  ctx: ToolContext,
): Promise<string> {
  const db = getDatabase();
