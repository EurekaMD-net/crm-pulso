...
function findCuenta(nombre: string): { id: string; nombre: string } | undefined {
  const db = getDatabase();
  // Try exact first, then LIKE
  const exact = db.prepare('SELECT id, nombre FROM cuenta WHERE nombre = ?').get(nombre) as any;
  if (exact) return exact;
  const fuzzy = db.prepare('SELECT id, nombre FROM cuenta WHERE nombre LIKE ?').get(`%${nombre}%`) as any;
  return fuzzy;
}

/** Fuzzy match propuesta by title, optionally scoped to a cuenta. */
function findPropuesta(titulo: string, cuentaId?: string): { id: string; titulo: string; ae_id: string; etapa: string } | undefined {
  const db = getDatabase();
  if (cuentaId) {
    const row = db.prepare('SELECT id, titulo, ae_id, etapa FROM propuesta WHERE titulo LIKE ? AND cuenta_id = ?').get(`%${titulo}%`, cuentaId) as any;
    if (row) return row;
  }
  return db.prepare('SELECT id, titulo, ae_id, etapa FROM propuesta WHERE titulo LIKE ?').get(`%${titulo}%`) as any;
}
...
export function registrar_actividad(args: Record<string, unknown>, ctx: ToolContext): string {
  const db = getDatabase();
  ...
  const fechaSiguienteAccion = args.fecha_siguiente_accion as string | undefined;

  const cuenta = cuentaNombre ? findCuenta(cuentaNombre) : undefined;
  if (cuentaNombre && !cuenta) {
    return JSON.stringify({ error: `No encontré la cuenta "${cuentaNombre}". Verifica el nombre.` });
  }

  const propuesta = propuestaTitulo ? findPropuesta(propuestaTitulo, cuenta?.id) : undefined;
...
export function crear_propuesta(args: Record<string, unknown>, ctx: ToolContext): string {
  const db = getDatabase();
  ...
  `).run(
    id, cuenta.id, ctx.persona_id, titulo,
    valorEstimado ?? null,
  ...
  const valor = valorEstimado ? ` por $${(valorEstimado / 1_000_000).toFixed(1)}M` : '';
  return JSON.stringify({ ok: true, id, mensaje: `Propuesta creada: "${titulo}" para ${cuenta.nombre}${valor}` });
}
...
export function actualizar_propuesta(args: Record<string, unknown>, ctx: ToolContext): string {
  const db = getDatabase();
  ...
  if (!propuesta) {
    return JSON.stringify({ error: `No encontré la propuesta "${propuestaTitulo}".` });
  }

  // Access check: AE can only update own, gerente+ can update team's
  if (propuesta.ae_id !== ctx.persona_id && !ctx.full_team_ids.includes(propuesta.ae_id)) {
  ...
  if ((etapa === 'perdida' || etapa === 'cancelada') && !razonPerdida) {
    return JSON.stringify({ error: `Se requiere razon_perdida para marcar como ${etapa}.` });
  }
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(etapa ?? null, valorEstimado ?? null, notas ?? null, razonPerdida ?? null, timestamp, propuesta.id);

  const changes: string[] = [];
  if (etapa) changes.push(`Etapa: ${propuesta.etapa} → ${etapa}`);
  if (valorEstimado) changes.push(`Valor: $${(valorEstimado / 1_000_000).toFixed(1)}M`);

  return JSON.stringify({ ok: true, id: propuesta.id, mensaje: `Propuesta "${propuesta.titulo}" actualizada. ${changes.join(', ')}` });
}
...
export function cerrar_propuesta(args: Record<string, unknown>, ctx: ToolContext): string {
  ...
  if ((resultado === 'perdida' || resultado === 'cancelada') && !razon) {
    return JSON.stringify({ error: `Se requiere razón para marcar como ${resultado}.` });
  }
...
export function actualizar_descarga(args: Record<string, unknown>, ctx: ToolContext): string {
  const db = getDatabase();
  const cuentaNombre = args.cuenta_nombre as string;
  ...
  const cuenta = findCuenta(cuentaNombre);
  if (!cuenta) {
    return JSON.stringify({ error: `No encontré la cuenta "${cuentaNombre}".` });
  }

  // Try update existing, or insert new
  const existing = db.prepare('SELECT id FROM descarga WHERE cuenta_id = ? AND semana = ? AND año = ?').get(cuenta.id, semana, año) as any;

  if (existing) {
    db.prepare('UPDATE descarga SET notas_ae = ? WHERE id = ?').run(notasAe, existing.id);
    return JSON.stringify({ ok: true, mensaje: `Notas de descarga actualizadas para ${cuenta.nombre} semana ${semana}.` });
  }

  const id = genId('desc');
  db.prepare('INSERT INTO descarga (id, cuenta_id, semana, año, notas_ae) VALUES (?, ?, ?, ?, ?)').run(id, cuenta.id, semana, año, notasAe);
  return JSON.stringify({ ok: true, id, mensaje: `Descarga creada para ${cuenta.nombre} semana ${semana} con notas.` });
}
