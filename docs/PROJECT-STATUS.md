...
> Last updated: 2026-04-26 (cost_ledger + backup + full audit + 5 audit-fix batches + §2 security batch landed and pushed. Audit: 6-dimension parallel sweep, **120 raw findings** at `docs/AUDIT-2026-04-26.md` — all 5 non-deferred batches landed (§5/§4/§1/§3/§6) plus §2 deferred security items B1+B2+B4+A5: aprobaciones APPROVAL_TABLES allowlist, Drive query escape with backslash+quote ordering, INFERENCE_PRIMARY_URL SSRF guard (https-only, hostname allowlist, IPv4+IPv6 private-range rejection, IDN/integer/hex bypass-pinned), DASHBOARD_JWT_SECRET 32-char floor. Audit-on-audit caught a dead IPv6 check + missing bypass tests, both fixed pre-commit. **All 1164 tests, 64 test files green** (1130 baseline + 34 new). Container rebuilt and restarted clean. **Wiring gap discovered**: `NODE_ENV` not set on systemd unit, so B4/A5 prod-only checks no-op until `Environment=NODE_ENV=production` added. Remaining queue (~2h) in `docs/AUDIT-FOLLOWUP-2026-04-26.md`: B3 PG `~/.pgpass` migration (split per user), A1+A2 docstrings, A3 + S1 tests. Pushed: `34411c0`, `6703f85`, `7c8faa9`, `c0bd6f9`, `6afe88d`, `1e4ba9a`, plus tonight's §2 batch. Earlier: `53f42da`, `a923eff`, `a38b228`, `24af7e8`, `4d0cfda`, `85475d8`)
