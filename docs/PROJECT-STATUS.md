...
> Last updated: 2026-04-26 (cost_ledger bug chain + backup system + full audit. Cost: SSE usage capture, per-call recordCost in `infer()` so sentiment + summarizer are no longer invisible, hard monthly-budget guard, `GET /api/v1/budget`, vitest test isolation via per-run `CRM_DB_PATH`, `INFERENCE_TOKEN_BUDGET` 25k→80k, honest exit messages. Backup: shipped Component A (15-min bytea snapshots into Supabase PG `crm_backup_meta.crm_backups`, gzipped + sha256, 90-day retention) + Component B (daily 03:00 MX `pgloader` mirror at `crm_mirror.*`, atomic schema-swap). Audit: 6-dimension parallel agent sweep, **120 raw findings** at `docs/AUDIT-2026-04-26.md` with §7 fix-order (5 batches) — top items: doom-loop Map LRU bug, Promise.all-not-allSettled, dateCutoff UTC-into-MX, dashboard N+1, getCurrentWeek week-boundary cascade (greens 9 failing tests). 71 tools, 29 tables, **1130 tests, 63 test files**, 9 pre-existing failures explained by §5 `getCurrentWeek` cascade. row #28 verified end-to-end (`qwen3.6-plus / 26012p+635c / $0.022`). Commits: `53f42da`, `a923eff`, `a38b228`, `24af7e8`, `4d0cfda`)
