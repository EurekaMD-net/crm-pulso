...
> Last updated: 2026-04-26 (cost_ledger bug chain + backup system. Cost: SSE usage capture, per-call recordCost in `infer()` so sentiment + summarizer are no longer invisible, hard monthly-budget guard, `GET /api/v1/budget` endpoint, vitest test isolation via per-run `CRM_DB_PATH`, `INFERENCE_TOKEN_BUDGET` 25k→80k so it backstops compression, honest exit messages distinguishing budget vs. round exhaustion. Backup: live SQLite was previously single-point-of-failure with no automation — shipped Component A (15-min bytea snapshots into Supabase PG `crm_backup_meta.crm_backups`, gzipped + sha256, 90-day retention, byte-perfect restore verified) + Component B (daily 03:00 MX `pgloader` schema-translated mirror in `crm_mirror.*` schema, 29 tables/3936 rows, atomic schema-swap so readers never see a half-loaded mirror). Both via systemd timers, ops via `crm-backup` / `crm-restore` / `crm-backup-list` / `crm-mirror`. 71 tools, 29 tables, **1130 tests, 63 test files**, 9 pre-existing failures unchanged. row #28 verified end-to-end (`qwen3.6-plus / 26012p+635c / $0.022`). Commits: `53f42da`, `a923eff`, `a38b228`, `24af7e8`)
