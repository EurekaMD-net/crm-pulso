...
> Last updated: 2026-04-26 (engine ownership Phases 1 + 2a + 2b shipped. Engine declared a permanent fork (upstream NanoClaw diverged into v2 architecture). Phase 1: deleted 86 vestigial upstream artifacts, rewrote engine/CLAUDE.md + root CLAUDE.md, persisted 3-phase plan. Phase 2a: container resource limits configurable via env (`CONTAINER_MEMORY`/`CPUS`/`PIDS_LIMIT`, defaults 512m/1/256 — preserves prior hardcoded behavior + adds `--pids-limit` closing audit gap; `'0'` skips flag; `trimEnv()` handles empty/whitespace edge case). **Phase 2b (this commit)**: extracted bootstrap sequence from `engine/src/index.ts:main()` into new `engine/src/bootstrap.ts` (~85 LOC) exporting `bootstrapEngine()`. Same load-bearing order, same fail-fast `try/catch` around bootstrapCrm. index.ts shrinks ~25 lines + 11 imports cleaned up. Scope smaller than original plan implied — closer reading showed no bigger restructure was warranted. Phase 2 Item 4 (trim engine/ipc.ts) closed N/A. Phase 2c (heartbeat reaper) queued. Audit fully closed. **All 1175 tests, 66 test files green** (1166 baseline + 9: 7 Phase 2a + 2 Phase 2b). Only deferred items per audit §7 are `BUDGET_ENFORCE` default flip + container resource limits as a separate per-deployment tuning surface — both pending user discussion. Pushed: `34411c0`, `6703f85`, `7c8faa9`, `c0bd6f9`, `6afe88d`, `1e4ba9a`, `99954ff`, `3f4d386`, `799b6b9` (P1), `24dcec6` (P2a), plus this Phase 2b commit. Earlier: `53f42da`, `a923eff`, `a38b228`, `24af7e8`, `4d0cfda`, `85475d8`)
