...
> Last updated: 2026-04-26 (engine ownership Phase 2 fully shipped. Engine is a permanent fork. Phase 1: deleted 86 vestigial artifacts. Phase 2a: container resource limits via env (`CONTAINER_MEMORY`/`CPUS`/`PIDS_LIMIT`). Phase 2b: bootstrap sequence extracted to `engine/src/bootstrap.ts`. **Phase 2c (this commit, Option B)**: operator visibility into active containers — `engine/src/group-queue.ts` gained `startedAt` + `getActiveContainers()` snapshot method; new `engine/src/container-stats-logger.ts` logs `'container active'` every 5 min while any container runs; new localhost-only `GET /api/v1/containers/active` dashboard endpoint. Picked Option B over the originally-planned full heartbeat reaper after reading container-runner.ts:537 — existing `IDLE_TIMEOUT + reset-on-stream-output` already acts as effective heartbeat; the actual gap was just operator visibility for the rare wedge case. Verified post-deploy: localhost returns snapshot, public IP returns 403. Audit-on-audit caught 2 belt-and-suspenders Lows (try/catch around getActiveContainers in the logger, JSDoc clarifying "active = engine still tracks the child handle"); both fixed pre-commit. Phase 2 fully done — all 4 items addressed (one closed N/A). Audit fully closed. **All 1183 tests, 67 test files green** (1166 baseline + 17 across Phase 2: 7 P2a + 2 P2b + 8 P2c). Only deferred items per audit §7 are `BUDGET_ENFORCE` default flip + container resource limits as a separate per-deployment tuning surface — both pending user discussion. Pushed: `34411c0`, `6703f85`, `7c8faa9`, `c0bd6f9`, `6afe88d`, `1e4ba9a`, `99954ff`, `3f4d386`, `799b6b9` (P1), `24dcec6` (P2a), `3ee0c7e` (P2b), plus this P2c commit. Earlier: `53f42da`, `a923eff`, `a38b228`, `24af7e8`, `4d0cfda`, `85475d8`)
